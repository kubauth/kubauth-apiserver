...
.PHONY: build
build:  display ## Build kubauth-apiserver binary
	CGO_ENABLED=0 go build -ldflags '-X kubauth-apiserver/internal/global.Version=$(APP_VERSION) -X kubauth-apiserver/internal/global.BuildTs=$(BUILD_TS)' -o bin/kubauth-apiserver main.go
...
docker-ubuntu-build: check-registry ## Build docker image using Ubuntu 22.04 as base
...
docker-buildx: check-registry display  ## Build and push docker image for the manager for cross-platform support
	...
	- $(CONTAINER_TOOL) buildx build --builder kubauth-apiserver-builder --push --platform=$(PLATFORMS) --build-arg VERSION=$(APP_VERSION) --build-arg BUILD_TS=$(BUILD_TS) --tag $(IMG_REPO):$(APP_VERSION) -f Dockerfile .
